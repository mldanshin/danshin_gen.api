...
final class PersonStoredPhotoControllerTest extends TestCase
{
    ...
    public function testStore200(?array $photo, callable $expected): void
    {
        ...
        return [
            ...
            [
                [
                    [
                        "order" => 1,
                        "file" => UploadedFile::fake()->image("test1.png")
                    ],
                    [
                        "order" => 3,
                        "date" => "20??-01-30",
                        "file" => UploadedFile::fake()->image("test1.jpeg")
                    ],
                    [
                        "order" => 2,
                        "date" => "20??-01-13",
                    ...
                    [
                        "order" => 4,
                        "date" => "2024-08-22",
                        "file" => UploadedFile::fake()->image("test1.webp")
                    ]
                ],
                function ($object, $response, $disk) {
                    $personId = $response["person_id"];
                    $photo = PhotoEloquentModel::where("person_id", $personId)->orderBy("order")->get();
                    $object->assertCount(4, $photo);
                    $object->assertCount(4, $disk->allFiles("photo/$personId"));
                    ...
                    $object->assertEquals("webp", (new \SplFileInfo( $disk->allFiles("photo/$personId")[3]))->getExtension());
                    ...
                    $object->assertEquals("20??-01-30", $photo[2]->date);

                    $object->assertEquals(4, $photo[3]->order);
                    $object->assertEquals("2024-08-22", $photo[3]->date);
                }
            ],
        ];
    }
