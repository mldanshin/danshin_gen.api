...
final class PersonEditControllerTest extends TestCase
{
    ...
    public function testEdit200(string $id, callable $expected): void
    {
            ...
            [
                ...
                fn (AssertableJson $json) =>
                    $json->where("id", 5)
                        ...
                        ->where("surname", "Danshin")
                        ->where("name", "Maxim")
                        ->has('oldSurname.0', fn ($json) => 
                            $json->where('order', 1)
                                ->where('surname', "Fake")
                        )
                        ->has('oldSurname.1', fn ($json) => 
                            $json->where('order', 2)
                                ->where('surname', "AFake")
                        )
                        ...
                        ->where('residences', [])
                        ->has('parents.0', fn ($json) =>    
                            $json->where('person', 3)
                                ->where('role', 2)
                        )
                        ->where('parents.0.role', 2)
                        ->has('parents.1', fn ($json) =>    
                            $json->where('person', 4)
                                ->where('role', 3)
                        )
                        ->has('marriages.0', fn ($json) => 
    ...
    public function testEdit401(): void
    {
        $response = $this->getJson(route("person.edit", ["person" => "1"]));
        $response->assertStatus(401);
    }

    public function testEdit403(): void
    {
    ...
    public function testEdit404(string $id): void
    {
        $response = $this->getJson(route("person.edit", ["person" => $id]), $this->getHeaderAdminToken());
