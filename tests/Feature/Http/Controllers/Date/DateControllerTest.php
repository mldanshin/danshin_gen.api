...
final class DateControllerTest extends TestCase
{
    ...
    public function testUpcoming200(string $request, callable $expected): void
    {
        ...
        return [
            [
                ...
                fn (AssertableJson $json) =>
                    $json->has('pastBirth', 1)
                        ->has('pastBirth.0', fn ($json) =>   
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "1964-08-21")
                                    ->etc()
                            )
                            ->has('person', fn ($json) =>   
                                $json->where('id', 10)
                        ...
                        ->has('pastBirthWould.0', fn ($json) =>   
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "1999-08-21")
                                    ->etc()
                            )
                            ->has('person', fn ($json) =>   
                                $json->where('id', 16)
                        ...
                        ->has('todayBirth', 1)
                        ->has('todayBirth.0', fn ($json) =>   
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "2012-08-22")
                                    ->etc()
                            )
                            ->has('person', fn ($json) =>   
                                $json->where('id', 12)
                        ...
                        ->has('todayBirthWould.0', fn ($json) =>   
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "2000-08-22")
                                    ->etc()
                            )
                            ->has('person', fn ($json) =>   
                                $json->where('id', 17)
                        ...
                        ->has('nearestBirth', 2)
                        ->has('nearestBirth.0', fn ($json) => 
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "1982-08-23")
                                    ->etc()
                            )
                            ->has('person', fn ($json) => 
                                $json->where('id', 14)
                        ...
                        ->has('nearestBirth.1', fn ($json) => 
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "2002-08-24")
                                    ->etc()
                            )
                            ->has('person', fn ($json) => 
                                $json->where('id', 18)
                        ...
                        ->has('nearestDeath', 1)
                        ->has('nearestDeath.0', fn ($json) => 
                            $json
                            ->has('date', fn ($json) =>   
                                $json->where('string', "2005-08-23")
                                    ->etc()
                            )
                            ->has('person', fn ($json) => 
                                $json->where('id', 17)
    ...
    public function testUpcoming401(): void
    {
    ...
    public function testUpcoming422(string $request): void
    {
        $response = $this->getJson(route("dates.upcoming") . $request, $this->getHeaderUserToken());
