...
use App\Models\Person\Editor\Updated\Internet;
use App\Models\Person\Editor\Updated\Marriage;
use App\Models\Person\Editor\Updated\OldSurname;
use App\Models\Person\Editor\Updated\ParentModel;
use App\Models\Person\Editor\Updated\Person;
use App\Models\Person\Editor\Updated\Photo;
use App\Models\Person\Editor\Updated\Residence;
...
final class PersonUpdatedRequest extends Request
{
    use Phone;
    ...
    public function rules(): array
    {
        return [
            ...
            "emails.*" => [
                ...
                "email",
                "distinct",
                Rule::unique('emails', "name")->ignore($this->input("id"), "person_id")
            ],
            ...
            "phones.*" => [
                ...
                "distinct",
                new PhoneRule,
                Rule::unique('phones', "name")->ignore($this->input("id"), "person_id")
            ],
            ...
            "photo.*.file" => "required_with:photo|image"
        ];
    }
                    ...
                    if (!empty($this->file("photo"))) {
                        ...
                        foreach ($files as $file) {
                            if (!$file["file"]->isValid()) {
                                $validator->errors()->add(
                                    'photo',
                                    __('validation_app.photo.file.format')
                                );
                            }
                        }
                    }
        ...
        if ($input === null || $files === null) {
            ...
            for ($i = 0; $i < $count; $i++) {
                $collect->push(
                    new Photo(
                        $input[$i]["order"],
                        isset($input[$i]["date"]) ? $this->getDate($input[$i]["date"]) : null,
                        $files[$i]["file"]
                    )
                );
            }
